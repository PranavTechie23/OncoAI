...
export function BentoGridItem({
  ...
  return (
    <div
      ...
      className={`relative group/bento row-span-1 rounded-2xl overflow-hidden bg-card/60 backdrop-blur-xl border border-border/50 dark:border-slate-800/60 hover:shadow-2xl transition-all duration-500 flex flex-col justify-between p-6 ${className}`}
    >
      <div
      ...
      <div className="relative z-10 flex flex-col h-full justify-between">
        ...
        <div className="mt-auto transition-transform duration-300 group-hover/bento:-translate-y-1">
          {icon && (
            <div className={`mb-4 inline-flex items-center justify-center h-12 w-12 rounded-xl bg-${color}/10 dark:bg-${color}/20 text-${color} transition-transform duration-300 group-hover/bento:scale-110`}>
              {icon}
            </div>
          )}
            ...
            <div className="opacity-0 translate-y-2 group-hover/bento:opacity-100 group-hover/bento:translate-y-0 transition-all duration-300">
              ...
              >
                <Link to={(isAuthenticated || isPublic) ? link : "/login"}>
                  ...
                  <span className="inline-block transition-transform duration-300 group-hover/bento:translate-x-1">
